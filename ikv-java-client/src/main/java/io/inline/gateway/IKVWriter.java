...
public class IKVWriter {
    private static final FieldValue BROADCAST_KEY = FieldValue.newBuilder().build();  // empty no op key
    ...
    public void publishFieldSchemaUpdates(UserStoreContext context, Collection<FieldSchema> newFieldsToAdd) throws InterruptedException {
        Preconditions.checkNotNull(context);
        ...
        IKVDataEvent event = IKVDataEvent.newBuilder()
                .setEventHeader(EventHeader
                        .newBuilder()
                        .build())
                .setUpdateFieldSchemaEvent(UpdateFieldSchemaEvent.newBuilder()
                        .addAllNewFieldsToAdd(newFieldsToAdd)
                        .build())
                .build();
