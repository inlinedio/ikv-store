...
public class Worker {
    ...
    public void build(String accountId, String storeName) throws InvalidProtocolBufferException {
        ...
        if (maybeContext.isEmpty()) {
            // Invalid args
            LOGGER.error("Invalid store args for offline index build, " +
                    "accountid: {} storename: {}", accountId, storeName);
            return;
        }
        ...
        IKVStoreConfig config = IKVStoreConfig.newBuilder()
                .mergeFrom(sotConfigs)
                .putStringConfigs(IKVStoreConfigConstants.MOUNT_DIRECTORY, mountDirectory)
                .build();

        LOGGER.info("Starting offline build for accountid: {} storename: {} config: {}",
                accountId, storeName, config);

        try {
            IKVClientJNI.buildIndex(config.toByteArray());
            LOGGER.info("Successfully finished offline build for accountid: {} storename: {}",
                    accountId, storeName);
        } catch (Exception e) {
            LOGGER.error("Error during offline build for accountid: {} storename: {}. Error: ",
                    accountId, storeName, e);
            return;
        }
    ...
    private static String mountDirectory(String accountId, String storeName) {
        ...
        return String.format("%s/%s/%s/%d/%d", WORKING_DIR, accountId, storeName, currentEpochMillis, 0);
    }
}
